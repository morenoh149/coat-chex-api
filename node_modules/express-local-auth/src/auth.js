  ...
  function buildAuthService(userStore) {
    return {
      ...
      hash: function (unhashed, cb) {
        ...
        bcrypt.hash(unhashed,
                    genSaltRounds,
                    function (hashingErr, hashed) {
          if (hashingErr) {
      ...
      /*
       * Note: long name to indicate that
       * *no* authentication is done in this method
       */
      markLoggedInAfterAuthentication: function (req, user, cb) {
        req.logIn(user, function (err) {
          ...
          if (userStore.logIn) {
            ...
            verifyEmailTokenStore.findByEmail(user.email, function(err,
                                                                   found){
              if (err || found) {
                logger.error('Error user has not verified email',
                             user.email, err);
                cb('Error user has not verified email');
              } else {
      ...
      logOut: function (req, user, cb) {
        ...
        if (user && userStore.logOut) {
          ...
          logger.info('no user detected, no actions necessary');
          cb(null);
        } else {
    ...
    passport.use(new LocalStrategy({
      ...
      function verify(email, password, done) {
        ...
        userStore.findByEmail(email, function (err, user) {
          if (err) {
            logger.error('Error finding user "%s" in user store in verify fn',
                         email,
                         err);
            return done(err);
          }
          if (!user) {
            logger.info('Could not find user "%s" in user store in verify fn',
                        email);
            return done(null, false, { userNameValid: false });
          }

          var verifyPassword = function () {
            authService.verifyHash(password,
                                   user.hashedPassword,
                                   function (err, verifiied) {
              if (err) {
                logger.error(
                  'Error verifying password hash for user "%s" in verify fn',
                  user.email,
                  err);
                return done(err);
              }
              ...
              if (!verifiied) {
                ...
                if (user.failedLoginAttempts >= options.failedLoginsBeforeLockout) {
                  ...
                  logger.info('Incorrect password for user "%s" in verify fn. user.failedLoginAttempts=',
                              user.email,
                              user.failedLoginAttempts);
                }
                ...
                userStore.update(user, function (err) {
                  if (err) {
                    logger.error('Error updating failed login attempt count for user "%s" in verify fn',
                                 user.email,
                                 err);
                    return done(err);
                  }
                ...
                if (user.failedLoginAttempts > 0) {
                  ...
                  // happy path
                  logger.debug('Successfully verified user "%s" in verify fn 1', user.email);
                  return done(null, user);
                }
              }
            });
    ...
    var handlers = {
        ...
        return function loginHandler(req, res, next) {
          ...
          passport.authenticate('local', function (err, user, info) {
            ...
            if (!user) {
              ...
              // if token for this email is in token store
              // then user hasn't verified email
              // verifyEmailTokenStore.findByEmail(user.email, function(err, user){
              //   if(err){
              //   next(err);
              //   } else {
              //   console.log('user is: ', user);
              //   logger.error('Error user has not verified via email');
              //   res.status(403)
              //   .send('Please activate account by clicking the link we sent your email address ' + user.email);
              //   }
              // })
              console.log('email tokenStore from auth:login', verifyEmailTokenStore.getTokens());
              authService.markLoggedInAfterAuthentication(req,
                                                          user,
                                                          function (err) {
                  if(err){
      ...
      logout: function () {
        return function logoutHander(req, res, next) {
          authService.isAuthenticated(req, function (err,
                                                     authenticatedUser) {
            if (err) {
              return next(err);
            }
