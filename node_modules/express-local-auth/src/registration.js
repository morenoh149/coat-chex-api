...
module.exports = function(sharedServices, authService, options) {
  ...
  var routeHandlers = {
    register: function (routeOptions) {
      ...
      return function registerHandler(req, res, next) {
        req.checkBody('email', 'Valid email address required')
          .notEmpty().isEmail();
        ...
        var email = options.normalizeCase ?
        ...
        authService.hash(userDetails.password,
                         function (err, hashedPassword) {
          if (err) {
            logger.error(
              'Error hashing password while registering user "%s"',
              email,
              err);
            return next(err);
          }

          // Make sure no possibility of storing unhashed password
          delete userDetails.password;

          userDetails.hashedPassword = hashedPassword;

          userStore.add(userDetails,
                        function (err, userAlreadyExists, user) {
            if (err) {
              logger.error(
                'Error adding user "%s" to userStore',
                email,
                err);
              return next(err);
            }
            if (userAlreadyExists) {
              logger.info(
                'Registration details for user "%s" already exist',
                email);
              return utils.handleError(
                'Registration details already in use',
                errorCfg,
                409)(req, res, next);
            }

  // refactor to send registration email and not login
  ...
  //   });
  // };
            var sendRegEmail = function (verifyQueryString) {
              emailService.sendRegistrationEmail(user,
                                                 verifyQueryString,
                                                 function(err){
              if(err) {
                // log error but continue
                logger.error('Error sending registration email to "%s"',
                             email,
                             err);
              }
              logger.info('Successfully registered user "%s"', email);
            ...
            if (options.verifyEmail) {
              ...
              authService.hash(unhashedToken,
                               function (err, hashedToken) {
                if (err) {
                  logger.error('Error hashing verify email token for user "%s" during registration', email, err);
                  return next(err);
                }

                tokenObj.hashedToken = hashedToken;

                console.log('token store before addition:', verifyEmailTokenStore.getTokens());
                verifyEmailTokenStore.add(tokenObj, function (err) {
                  ...
                  console.log('token store after addition:', verifyEmailTokenStore.getTokens());

                  // sendRegEmailAndLogIn(verifyQueryString);
                  sendRegEmail(verifyQueryString);
                });
              });
            } else {
    ...
    /*
     * updated to remove check and remove token from tokenstore
     */
    verifyEmailView: function () {
      return function verifyEmailAddressHandler(req, res, next) {
        ...
        req.checkQuery('token',
                       'Verify email token required'
                      ).notEmpty();

        var errorCfg = utils.getErrorConfig(options,
        ...
        findAndVerifyRegistrationEmailToken(email,
                                            token,
          ...
          if (verified) {
            userStore.findByEmail(email, function (err, user) {
              ...
              userStore.update(user, function (err) {
                ...
                verifyEmailTokenStore.removeAllByEmail(email,
                                                       function (err) {
                  if (err) {
            ...
            var useRedirect = false;
            utils.handleError('Unknown or invalid token',
                              useRedirect,
                              400)(req, res, next);
          }
        });
  ...
  // this function should check and remove token from token storage
  function findAndVerifyRegistrationEmailToken(email, unhashedToken, cb) {
    verifyEmailTokenStore.findByEmail(email, function(err, tokenDetails) {
      ...
      if (!tokenDetails) {
        logger.info('Could not find verify email token using email "%s"',
                    email);
       ...
       * in callback verified is true if hash is correct
       */
      authService.verifyHash(unhashedToken,
